...
try:
    ...
    # This will print the error to the Vercel logs for debugging
...
def get_parking_data():
    ...
    try:
        ...
        for row in data:
            ...
            parking_name_en = re.sub(r'^\d+\)\s*', '', parking_name_raw)

            processed_lot = {
                'ParkingLotID': cleaned_id,
                'Parking_name_en': parking_name_en,
                'Parking_name_ta': parking_name_en, # No Tamil support as requested
                'Location_Link': '#', # No map support as requested
                'Notes_en': '', 'Notes_ta': ''
            }
...
def api_data():
    ...
    # Filter out VIP lots from the main dashboard view
    filtered_lots = [lot for lot in all_lots.values() if lot.get('Route_en') != 'VIP']
    response = {
...
def overall_history():
    ...
    for route in routes:
        final_datasets.append({
            ...
            "pointRadius": 0,
            "borderWidth": 2.5 # Make lines thicker
        })
...
def parking_lot_history():
    ...
    dataset = {"label": 'Occupancy (%)', "data": graph_data, "borderColor": 'rgba(255, 159, 64, 1)', "backgroundColor": 'rgba(255, 159, 64, 0.2)', "fill": True, "tension": 0.1, "pointRadius": 1, "pointHoverRadius": 5}
