...
def get_parking_data():
    ...
    # Define a mapping from the sheet's route codes to full names
    ROUTE_MAP = {
    ...
    try:
        # Use numericise_ignore to prevent gspread from converting values
        data = live_sheet.get_all_records(numericise_ignore=['all'])
        all_lots_data = {}

        for row in data:
            # Skip rows without a valid ParkingLotID
            ...
            # Create a new, clean dictionary for this parking lot
            processed_lot = {}
            processed_lot['ParkingLotID'] = cleaned_id

            # --- Map New Sheet Columns to Expected JSON Keys ---

            # Safely convert numeric values, defaulting to 0 on error
            try:
            ...
            processed_lot['Parking_name_ta'] = row.get('Parking Name_ta', processed_lot['Parking_name_en']) # Fallback to English name
            processed_lot['Notes_en'] = row.get('Notes_en', '')
            processed_lot['Notes_ta'] = row.get('Notes_ta', processed_lot['Notes_en']) # Fallback to English notes
            ...
            # Calculate occupancy percentage, safely handling division by zero
            if total_capacity > 0:
                processed_lot['Occupancy_Percent'] = (current_vehicles / total_capacity) * 100
            else:
                processed_lot['Occupancy_Percent'] = 0

            # Map other numeric fields
            for key_new, key_old in [('In', 'CurrentIn'), ('Out', 'CurrentOut'), ('Available Space', 'AvailableSpace')]:
                 try:
                    processed_lot[key_old] = int(row.get(key_new, 0))
                 except (ValueError, TypeError):
                    processed_lot[key_old] = 0

            # Map and convert geographic data (CRITICAL for map view)
            ...
            # Use the cleaned ID as the key for the final dictionary
...
# --- Flask Routes (API Endpoints) - NO CHANGES NEEDED BELOW THIS LINE ---
...
def overall_history():
    ...
    id_to_route_map = {
        lot_id: data['Route_en'] 
        for lot_id, data in live_parking_data_dict.items()
    }
    ...
    colors = {
        "Thoothukudi": "rgba(255, 99, 132, 1)",
        "Tirunelveli": "rgba(54, 162, 235, 1)",
        "Nagercoil": "rgba(75, 192, 192, 1)"
    }
    
    final_datasets = []
    for route in routes:
        final_datasets.append({
            "label": f'{route} Vehicle Count',
            "data": datasets[route],
...
def parking_lot_history():
    ...
    try:
        ...
        print(f"Error fetching history from Google Sheet: {e}")
        return jsonify({"error": "Could not fetch history data"}), 500
    ...
    for record in all_history:
        ...
        try:
            ...
            if record_datetime >= time_24_hours_ago:
                graph_data.append({
                    "x": record_datetime.isoformat(),
                    "y": float(record.get('Occupancy_Percent', 0))
                })
        except (ValueError, TypeError, KeyError):
            continue
    ...
    dataset = {
        ...
        "fill": True, "tension": 0.1, "pointRadius": 1, "pointHoverRadius": 5
    }
